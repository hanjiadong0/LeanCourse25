...
example {α : Type*} [PartialOrder α]
  ...
  have := isDense x y hxy
  obtain ⟨z, ⟨hxz, hzy⟩⟩ := this
  specialize isDense x z hxz
  obtain ⟨z', hxz', hz'z⟩ := isDense
  use z', z
  done
