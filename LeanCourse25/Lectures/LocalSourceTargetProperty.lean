...
lemma mk_of_continuousAt (hf : ContinuousAt f x)
  ...
  exact ⟨domChart.restr s, codChart, by simp [hsopen.interior_eq, hxs]; grind, hfx,
    restr_mem_maximalAtlas (contDiffGroupoid n I) hdomChart hsopen, hcodChart,
    by simp; grind [interior_subset],
...
lemma congr_of_eventuallyEq (hP : IsLocalSourceTargetProperty P)
  ...
  · exact hP.congr (hfg.mono hss' |>.mono (by simp; grind [interior_subset]))
      <| hP.mono_source hs hf.property
...
lemma prodMap [IsManifold I n M] [IsManifold I' n M'] [IsManifold J n N] [IsManifold J' n N']
  ...
  · -- This sorry and the next are proven in mathlib: we don't copy over the lemma.
    sorry --exact IsManifold.mem_maximalAtlas_prod
      --(domChart_mem_maximalAtlas hf) (domChart_mem_maximalAtlas hg)
  · sorry -- apply IsManifold.mem_maximalAtlas_prod
