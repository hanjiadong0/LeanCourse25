...
The initial meeting (*Vorbesprechung*) is on Thursday, February 12 at 10.15 in room 0.001.
...
Each ordinal number has a canonical representative: the type of all ordinals which are strictly
smaller than it.
...
**Proof sketch.** Set `S` be a set of ordinal numbers.
...
- successor ordinals: "add a top element" (`WithTop` in Lean), i.e. WithTop \alpha
...
example : (Cardinal.mk ℕ) = Cardinal.aleph0 := by exact?
