...
-- the square brackets around `str` make that argument implicit in
-- the constructor for `MyGrpCat`.
example {G : Type u} [hG : Group G] : MyGrpCat :=
  ⟨G⟩
  -- { carrier := G }
#check MyGrpCat.mk
...
#synth Category GrpCat
#check GrpCat.instCategory.{0}
#synth Category Cat.{0, 0}
#check (Cat.category.{0, 0} : Category.{0, 1} Cat)
#print LargeCategory
...
example (X Y : Type u) : (X ≅ Y) ≃ (X ≃ Y) := equivEquivIso.symm
example (G H : GrpCat) : (G ≅ H) → (G ≃* H) := by exact fun a ↦ a.groupIsoToMulEquiv
example (G H : GrpCat) : (G ≃* H) → (G ≅ H) := by exact fun a ↦ a.toGrpIso
...
structure MyCone (F : J ⥤ C) where
  pt : C
  π : (const J).obj pt ⟶ F
  -- equivalently, `π` consists of
  -- π_obj : ∀ j : J, pt ⟶ F.obj j
  -- π_nat : ∀ {j j' : J} (f : j ⟶ j'),
  --   π_obj j ≫ F.map f = π_obj j'
...
class MyHasLimitsOfShape (J : Type w) [Category J] : Prop where
