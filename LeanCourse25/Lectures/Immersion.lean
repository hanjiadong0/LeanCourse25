...
lemma _root_.OpenPartialHomeomorph.extend_prod
    (f : OpenPartialHomeomorph M H) (f' : OpenPartialHomeomorph M' H') :
    (f.prod f').extend (I.prod I') = (f.extend I).prod (f'.extend I') := by simp

lemma _root_.Set.eqOn_prod_iff {α γ δ : Type*} {s : Set α} {a b : α → γ × δ} :
...
lemma isImmersionAt (h : IsImmersionAtOfComplement F I J n f x) :
  ...
  sorry -- proof in mathlib; skipped
...
lemma isImmersion (h : IsImmersionOfComplement F I J n f) : IsImmersion I J n f := by
  ...
  sorry -- proof is in mathlib; skipped
