...
-- We can prove this composition law from the Galois connection property.
...
example {X : Type*} {s t : Set X} : 𝓟 s ⊓ 𝓟 t = 𝓟 (s ∩ t) := by sorry
...
/- `f` is continuous at `x`: `\lim_{x → x₀} f(x) = y₀` -/
...
-- Stated in terms of the order on filters, this is equivalent to the following.at x".
example {f : X → Y} {x : X} :
