...
lemma foo (f : ℝ → ℝ) (u : ℕ → ℝ) (x₀ : ℝ)
    (hu : SequenceHasLimit u x₀)
    (hf : ContinuousAtPoint f x₀) :
    SequenceHasLimit (f ∘ u) (f x₀) := by
  unfold SequenceHasLimit
  -- let ε > 0 be arbitrary
  intro (ε : ℝ) (hε : ε > 0)
  -- use the fact that f is continuous at x₀ to find a δ > 0
  -- such that ∀ x, |x - x₀| < δ → |f x - f x₀| < ε
  obtain ⟨δ : ℝ, hδ : δ > 0,
    h : ∀ x, |x - x₀| < δ → |f x - f x₀| < ε⟩ := hf ε hε
  -- use the fact that u has limit x₀ to find an N such that
  -- ∀ n ≥ N, |u n - x₀| < δ
  obtain ⟨N : ℕ, hN : ∀ n ≥ N, |u n - x₀| < δ⟩ := hu δ hδ
  -- we claim that this N works
  use N
  -- so let n ≥ N be arbitrary
  intro (n : ℕ) (hn : n ≥ N)
  -- by hN, we know that |u_n - x₀| < δ
  have : |u n - x₀| < δ := hN n hn
  have : |f (u n) - f x₀| < ε := h (u n) this
