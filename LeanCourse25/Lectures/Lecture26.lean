...
- Smooth vector bundles generalize the tangent bundle:
  ...
  * admits local trivializations: every `x : M` has an open neighborhood `U`
    such that π⁻¹(U) is homeomorph to `U × F` for some vector space `F`
...
#check List.sum (List.map (fun x => x ^ 3) [1, 2, 3])
end
...
def unexpListMap : Unexpander
  | `(List.sum $a:term) =>
    match a with
    | `(List.map $f:term $l:term) =>
      match f with
      | `(fun $x:ident ↦ $f) => `(∑ $x in $l, $f)
      | _ => throw ()
    | _ => throw ()
  | _ => throw ()
...
/- The fact that Lean shadows variables instead of updating
shows that this function doesn't do what you might expect. -/
def newFunction (n : ℕ) : IO ℕ := do
...
/- Lean has *mutable variables* where you can update the value. -/
def newFunction' (n : ℕ) : IO ℕ := do
  let mut k := n
  for i in List.range 10 do
    k := k + i
...
/- `IO` is a so-called *monad*, which means it implements two functions:
`pure` and `bind`. -/
...
/- Here are two ways of writing `pure` -/
example (n : ℕ) : IO ℕ := do
  return n
...
/- Here are three ways of writing `bind` -/
example (f : IO ℕ) (g : ℕ → IO ℤ) : IO ℤ := do
  let n ← f
  let k ← g n
  return k
...
def eratosthenes (n : ℕ) : SieveMonad (List ℕ) := do
  ...
  for p in 2...n do
    ...
    if ← isPrime p then
      ...
      for k in ((2*p)...n).iter.stepSize p do
        setNotPrime k
