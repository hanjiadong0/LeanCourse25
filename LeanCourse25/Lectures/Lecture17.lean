...
* `ContDiff ℝ n f` states that `f` is `C^n`
  `ContDiff ℝ ω f` states that `f` is analytic
...
**Practical remark**: today during this class,
I will discuss projects with you while you are working on exercises.
-/
...
that extends `μ` as `m(A) = inf{μ(B) | A ⊆ B ∧ B is measurable}`.
...
example {s : ℕ → Set X} (hmeas : ∀ i, MeasurableSet (s i))
    ...
    μ (⋃ i, s i) = ∑' i, μ (s i) := by
  exact measure_iUnion hdis hmeas
...
example (μ ν : Measure ℝ) (h : μ ≤ ν) (a b : ℝ) :
    μ (Icc a b) ≤ ν (Icc a b) := by
  exact h (Icc a b)
...
example (n : ℕ) (x : Fin n → ℝ) (s : Set (Fin n → ℝ)) :
    volume ((· + x) '' s) = volume s := by
  simp?
