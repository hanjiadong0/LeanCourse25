...
The `#synth` command asks Lean "find this instance using typeclass search, and tell me how it is
called".
-/
#synth MulAction G G
...
example {g g' : G} : g • g' = g * g' := by simp only [smul_eq_mul]
...
-- Lean knows about the diagonal action of a group.
...
example (r : R) (x y : M) : r • (x + y) = r • x + r • y := by exact DistribSMul.smul_add r x y
example (r s : R) (x : M) : (r + s) • x = r • x + s • x := by exact Module.add_smul r s x -- sorry
example (r s : R) (x : M) : (r * s) • x = r • s • x := by exact mul_smul r s x -- sorry
example (x : M) : (0 : R) • x = 0 := by exact zero_smul R x -- sorry
example (x : M) : (1 : R) • x = x := by exact MulAction.one_smul x -- sorry
example (r : R) : r • (0 : M) = 0 := by exact MulActionWithZero.smul_zero r -- sorry
...
#check (⊥ : Submodule R M)
example {x : M} : x ∈ (⊥ : Submodule R M) ↔ x = 0 := by exact Submodule.mem_bot R

#check (⊤ : Submodule R M)
#check Submodule.mem_top

example (f : M →ₗ[R] N) : LinearMap.ker f = ⊥ ↔ Injective f := LinearMap.ker_eq_bot
...
example (f : M →ₗ[R] N) (hf : Bijective f) : M ≃ₗ[R] N := LinearEquiv.ofBijective f hf
...
example {ι : Type*} [Fintype ι] [DecidableEq ι]
    (b : Basis ι K V) (f : V →ₗ[K] V) : Matrix ι ι K := LinearMap.toMatrix b b f
...
example : ∃ a, End.HasEigenvector f a v := sorry
...
example (r : R) (f : Π i, M i) (i₀ : ι) (x : M i₀) :
  ...
  exact (update_smul r f i₀ x).symm
  done
