...
/- This is an equivalent definition of Monad.
The Lean definition contains some derived notions as well. -/
structure MyMonad (m : Type → Type) where
  pure : ∀ (α : Type), α → m α
...
#simp NatTerm.eval n (add (succ zero) var)
...
-/
#check (`(1 + 1) : TacticM (TSyntax `term))
...
elab "try_hard'" : tactic => do
  logInfo "Oops" <|> logInfo "Ok"

elab "try_hard''" : tactic => do
  try
    throwError "Oops"
  catch error => logInfo "Ok"
...
elab "print_info" : tactic => do
  logInfo m!"Current statement to prove: {indentExpr (← getMainTarget)}"
  logInfo m!"This is file {← getFileName}"
  -- you can use `evalTactic` to run a piece of syntax as a tactic.
