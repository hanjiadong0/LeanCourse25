...
Last time we discussed integration:
...
* The canonical measure on some types is called `volume`.
  In `ℝⁿ` this is the Lebesgue measure.
* `∀ᵐ x ∂μ, P x` means that `P` holds for almost all `x`
  (relative to `μ`)
...
example : ∀ᵐ x : ℝ ∂volume, Irrational x := by
  unfold Irrational
  refine Countable.ae_notMem ?_ volume
...
is measurable and it maps the measure `μ : Measure X` to `ν : Measure Y`,
i.e. `ν s = μ (f ⁻¹' s)` for all (measurable) sets `s`.
This is important for Ergodic theory. -/
#check MeasurePreserving f μ ν
variable {g : ℝ → ℝ} in
#check MeasurePreserving g
-- #check MeasurePreserving f
...
example : ¬ Integrable (fun _ ↦ 1 : ℝ → ℝ) := by
  ...
  rw [isFiniteMeasure_iff volume] at h
  simp at h
  done
...
Here we're changing the variable `x = f y`
-/
...
/- pp.beta prints expressions in a simplified form,
reducing terms of the form `(fun x ↦ b) a` to `b`, with
all occurrences of `x` replaced by `a`. -/
-- set_option pp.beta true in
def myFac : ℕ → ℕ :=
  Nat.rec (motive := fun _ ↦ ℕ) 1
    (fun n fac_n ↦ (n + 1) * fac_n)
...
set_option pp.beta true in
lemma myFac_pos (n : ℕ) : 0 < myFac n :=
  Nat.rec (motive := fun n ↦ 0 < myFac n)
  (by simp [myFac]) (fun n ih ↦ by simp [myFac_succ, ih]) n
