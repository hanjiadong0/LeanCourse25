...
example {M N : Type*} [Monoid M] [Monoid N] (x y : M) (f : M →* N) :
    f (x * y) = f x * f y := by
  exact f.map_mul x y
  -- or: exact MonoidHom.map_mul f x y
...
-- Note: abel only works for additive abelian groups, not multiplicative ones.
example {G : Type*} [CommGroup G] (x y z : G) : z * x * (y * z⁻¹ * x⁻¹) = y := by
  -- group makes some progress, but does not solve the goal.
  sorry -- abel
...
/- Note that a map between groups that preserves `*`
is automatically a group homomorphism. -/
example {G H : Type*} [Group G] [Group H] (f : G → H)
    (h : ∀ x y, f (x * y) = f x * f y) : G →* H := by
  exact MonoidHom.mk' f h
...
example {G : Type*} [Group G] (H : Subgroup G) {x y : G}
  ...
  exact mul_mem hx hy
  --exact (Subgroup.mul_mem_cancel_right H hy).mpr hx
...
example {G : Type*} [Group G] (H : Subgroup G) : Group H := H.toGroup
...
example {G : Type*} [Group G] (H : Subgroup G) : Group {x : G // x ∈ H} := by exact H.toGroup
...
example {G : Type*} [Group G] (H H' : Subgroup G) : H ⊓ H' ≤ H := inf_le_left

example {G : Type*} [Group G] (H H' : Subgroup G) :
    ((H ⊔ H' : Subgroup G) : Set G) = Subgroup.closure ((H : Set G) ∪ (H' : Set G)) := by
  rw [Subgroup.sup_eq_closure H H'] -- apply also works
  -- or: simp [Subgroup.closure_union]
...
/- Normal subgroups are stated using a `Normal` class. -/
...
example {G H : Type*} [Group G] [Group H] (f : G →* H) : f.ker.Normal where--:= by
  conj_mem := by--exact f.normal_ker
    sorry
...
example {G : Type*} [Group G] (H : Subgroup G) [H.Normal] : Group (G ⧸ H) := by infer_instance
...
example {G G': Type*} [Group G] [Group G']
  ...
  by exact QuotientGroup.map N N' φ h
...
example (hH : H.index = 2) : H.Normal := Subgroup.normal_of_index_eq_two hH
...
example {n : ℕ} : Nat.card (Equiv.Perm (Fin n)) = (n)! := by
  rw [Nat.card_perm, Nat.card_fin n]
...
-- and has index two (assuming α has more than one element).
example [Nontrivial α] : (alternatingGroup α).index = 2 := by
  apply alternatingGroup.index_eq_two

example : (alternatingGroup α).Normal := by sorry
